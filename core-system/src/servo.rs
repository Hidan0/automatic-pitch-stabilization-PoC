...
impl<'a> ServoSG90<'a> {
    pub fn new<C: LedcChannel, T: LedcTimer>(
        ...
        let timer_driver = LedcTimerDriver::<'a>::new(
            timer,
            &TimerConfig::default()
                .frequency(50.Hz())
        ...
        .map_err(|err| {
            Error::InitDriver(format!("Can not create LedcTimerDriver due to {}", err).into())
        })?;

        let driver = LedcDriver::new(channel, timer_driver, gpio).map_err(|err| {
            Error::InitDriver(format!("Can not create LedcDriver due to {}", err).into())
        })?;
    ...
    pub fn write_angle(&mut self, angle: u32) -> Result<()> {
        ...
        self.driver
            .set_duty(duty)
            .map_err(|err| Error::DutyWrite(format!("{err}")))?;
